...
One potentially interesting future artifact would be a fully verified ARM or RISC-V emulator. This could be useful for folks needing to write bits of inline assembly in Rust programs. You could first write your specifications in a lifted Rust version and add some refinement annotations. For example, here is a really simple contract on a program that moves the value `0` into register `r0`.
...
An example of the specification for `mov` on an ARM7 CPU would look something like this:
