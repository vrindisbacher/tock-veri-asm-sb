...
impl Armv7m {
    ...
    #[flux_rs::sig(
        fn (self: &strg Armv7m[@old_cpu], GPR[@reg]) 
            requires is_valid_write_addr(get_sp(old_cpu.sp, old_cpu.mode, old_cpu.control))
    ...
    #[flux_rs::sig(
        fn (self: &strg Armv7m[@old_cpu], SpecialRegister[@reg]) 
            requires is_valid_write_addr(get_sp(old_cpu.sp, old_cpu.mode, old_cpu.control)) && !is_control(reg)
